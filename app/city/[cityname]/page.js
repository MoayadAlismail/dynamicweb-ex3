...
export default function CityWeather({ params }) {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        const response = await fetch(`${API_URL}?q=${params.cityname}&appid=${API_KEY}&units=metric`);
        if (!response.ok) {
  ...
  const bgColor = `linear-gradient(180deg, lightblue, rgba(85, 156, 185, ${cloudiness}))`;
